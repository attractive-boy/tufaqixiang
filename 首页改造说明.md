...
本次改造将首页的"地标评分"模块由硬编码数据改为从携程API实时获取北京地标数据，展示真实的景点评分和信息。
...
- 将 `landmarks` 从静态数据改为动态获取
- 新增 `loadBeijingLandmarks()` 方法，在页面加载时调用API
...
2. **智能排序**：按评分降序排列，只展示有评分的地标
3. **容错机制**：如果API请求失败，自动降级使用默认数据
4. **加载提示**：请求数据时显示加载动画
5. **数据展示**：展示地标名称、评分（星级）、评论数
